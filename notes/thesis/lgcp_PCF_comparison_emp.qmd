...
ggplot(pcf_panel_df, aes(x = r, y = g, linetype = model)) +
  geom_line(linewidth = 0.9) +
...
# stop here if object not supplied
if (!exists("X_cscp")) {
  stop("Please create a simulated CSCP point pattern called `X_cscp` before running this chunk.")
}
...
The next section investigates this empirically by fitting each model to data 
generated from the other, and comparing the resulting fits and parameter estimates.
